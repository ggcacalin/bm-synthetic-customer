...
# Loading vector store and creating retriever
...
def submit_message(input_message):
  response = agent_executor.invoke({"input": input_message, "chat_history": chat_history})
  chat_history.extend(
      [
          ("user", input_message),
          ("ai", response['output']),
...
def main(input_message):
  ...
  response = submit_message(input_message)
  return response
